def cursos_por_estado(estado: str) -> str:
    ...
    def select_curso(sg_estado: str) -> str:
        curso = input("Digite o código do curso que deseja se inscrever: ").upper()
        while True:
            if curso_permi.get(curso):
                if estado in curso_permi[curso]:
                    break    
                print("O código selecionado deve estar entre os mostrados na tabela acima.")
            else:
                print("O valor digitado não correponde ao código de um curso")
            curso = input("Digite o código do curso que deseja se inscrever: ").upper()
        return curso
    estado = estado.upper()
    curso_permi = {
